...
begin // high(TPdfGDIComment)<$47 so it will never begin with GDICOMMENT_IDENTIFIER
  ...
  Winapi.Windows.GdiComment(MetaHandle,L+1,D);
end;
...
begin // high(TPdfGDIComment)<$47 so it will never begin with GDICOMMENT_IDENTIFIER
  ...
  Winapi.Windows.GdiComment(MetaHandle,L+2,D);
end;
...
begin // high(TPdfGDIComment)<$47 so it will never begin with GDICOMMENT_IDENTIFIER
  ...
  Winapi.Windows.GdiComment(MetaHandle,L+(1+sizeof(TRect)),D);
end;
...
begin
  ...
  lcfn := {$ifdef UNICODE}System.SysUtils.LowerCase{$else}WideLowerCase{$endif}(lcfn);
  // batang.ttc (Korean)
