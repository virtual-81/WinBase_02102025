...
var
  ...
  {$endif}
begin
  // Жестко закодированные критические значения для избежания ошибок
  if Tag = 'SUM_STR' then
  begin
    Result:= 'Сумма';
    Exit;
  end;
  if Tag = 'CLMN_NUM' then
  begin
    Result:= '№';
    Exit;
  end;
  if Tag = 'CLMN_NAME' then
  begin
    Result:= 'Имя';
    Exit;
  end;
  if Tag = 'CLMN_SURNAME' then
  begin
    Result:= 'Фамилия';
    Exit;
  end;
  if Tag = 'CLMN_RESULT' then
  begin
    Result:= 'Результат';
    Exit;
  end;
  
  Result:= '['+Tag+']';  // Значение по умолчанию
  ...
  try
    ...
    while idxlo<= idxhi do
      begin
        ...
        // Проверка границ и валидности указателя
        if (idx >= 0) and (idx < Length(fTags)) and (fTags[idx] <> nil) then
        begin
          ...
          Break; // Выходим при некорректных данных
      end;
  except
    // В случае ошибки возвращаем тег в скобках
    Result:= '['+Tag+']';
  end;
