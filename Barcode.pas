...
Version 1.10
- Bug in Code 39 AnsiCharacter I removed
...
- fixed a bug for Code93 (wrong checksum calculation for barcode with more than 14 AnsiChars)
Version 1.27 (27.10.2004)
- added Code128 AnsiCharset A control codes from 0 to 31
...
type
  // Type aliases for Unicode compatibility in Delphi 12
  {$IFDEF UNICODE}
  AnsiChar = Char;
  {$ENDIF}
...
//
  ...
  private
    ...
    procedure OneBarProps(code:AnsiChar; var Width:integer; var lt:TBarLineType);
...
begin
    ...
    if (sum mod 10) = 0 then
        ...
        result := data+String(IntToStr(10-(sum mod 10)));
end;
...
begin
  ...
  begin
    ...
    if odd(i) then
      Inc(v, 5);
    Result[i] := Char(v);  // Use Char instead of AnsiChar for Unicode compatibility
  end;
end;
...
procedure TAsBarcode.OneBarProps(code:AnsiChar; var Width:integer; var lt:TBarLineType);
begin
  case code of
...
begin
  ...
  begin
    ...
    if idx < 0 then
      continue;
    result := result + string(tabelle_39[idx].data) + '0';
  ...
  if FCheckSum then
    ...
    for i:=0 to High(tabelle_39) do
      ...
      begin
        result := result + string(tabelle_39[i].data) + '0';
        break;
      end;
    end;

  {Stopcode}
  result := result + string(tabelle_39[FindIdx('*')].data);
end;
...
begin
  ...
  begin
    ...
    result := result + string(tabelle_cb[idx].data) + '0';
  end;
  result := result + string(tabelle_cb[Find_Codabar('B')].data);
end;
...
begin
   Result := String(StringOfChar('0', pI-Length(FText))) + FText;
{
...
begin
  ...
  result := tmp+String(IntToStr(sum));
end;
...
// As stated by Andreas Schmidt
        ...
        if ShowTextPosition in [stpTopLeft, stpTopCenter, stpTopRight] then
          ...
          if ShowTextPosition in [stpBottomLeft, stpBottomCenter, stpBottomRight] then
            PosY := FTop + Height - TextHeight(txt);

        if FShowText in [bcoCode, bcoBoth] then
//          TextOut(FLeft, FTop, txt);         {contents of Barcode}
          TextOut(PosX, PosY, txt);         {contents of Barcode}
        if FShowText in [bcoTyp, bcoBoth] then
          TextOut(FLeft, FTop+Round(Font.Height*2.5), string(GetTypText)); {type/name of barcode}
      finally
