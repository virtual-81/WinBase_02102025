...
type
  ...
  TInputResultDialog = class(TForm)
    ...
    procedure leCompExit(Sender: TObject);
...
begin
  try
    ...
    // В случае ошибки очищаем компоненты и выходим
    lbGroups.Clear;
    cbEvent.Clear;
    lbShooters.Clear;
    fData:= nil;
...
begin
  ...
  // Подключаем обработчик для проверки результата
  edtComp.OnExit := leCompExit;
  UpdateLanguage;
  UpdateFonts;
...
begin
  try
    ...
    Result:= 0;  // в случае ошибки возвращаем 0
  end;
end;
}
...
begin
  ...
  try
    // Проверяем наличие разделителей (точка или запятая) или число в диапазоне 110-1090
    var text := Trim(edtComp.Text);
    var hasDecimalSeparator := (pos('.', text) > 0) or (pos(',', text) > 0);
    var hasImpliedDecimals := false;
    
    if not hasDecimalSeparator then
      begin
        var tempVal, tempErr: integer;
        val(text, tempVal, tempErr);
        // Числа от 110 до 1090 считаются имеющими десятые
        hasImpliedDecimals := (tempErr = 0) and (tempVal >= 110) and (tempVal <= 1090);
      end;
    
    ER.Competition10:= StrToFinal10(text);
    ER.fCompetitionWithTens:= hasDecimalSeparator or hasImpliedDecimals;
  except
...
begin
  try
    ...
    // Игнорируем ошибки при переходе фокуса
  end;
end;

procedure TInputResultDialog.leCompExit(Sender: TObject);
var
  text: string;
  tempVal, tempErr: integer;
begin
  try
    text := Trim(edtComp.Text);
    if text = '' then
      Exit;
      
    // Проверяем, если число без разделителя больше 1090
    if (pos('.', text) = 0) and (pos(',', text) = 0) then
      begin
        val(text, tempVal, tempErr);
        if (tempErr = 0) and (tempVal > 1090) then
          begin
            if MessageDlg('Внимание! Введенное значение ' + text + ' превышает максимальный результат 109.0.' + #13#10 +
                         'Возможно, вы имели в виду результат с десятыми?' + #13#10 +
                         'Например: ' + text + ' ? ' + IntToStr(tempVal div 10) + '.' + IntToStr(tempVal mod 10),
